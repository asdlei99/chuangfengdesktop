...
void QExcel::setBorder(int beginRaw, int endRaw, int beginColumn, int endColumn)
{
	QString merge_cell;
	merge_cell.append(QChar(beginColumn - 1 + 'A'));  //初始列
	merge_cell.append(QString::number(beginRaw));  //初始行
	merge_cell.append(":");
	merge_cell.append(QChar(endColumn - 1 + 'A'));  //终止列
	merge_cell.append(QString::number(endRaw));  //终止行
	...
	border->setProperty("Color", QColor(0, 0, 0));   //设置单元格边框色（蓝色）
}
...
void QExcel::setCellString(const QString& cell, const QString& value, const EnumTextAliPos& posType, int fontSize, bool bBold)
{
	setCellTextPos(cell, posType);
	setCellFontSize(cell, fontSize);
	setCellFontBold(cell, bBold);
	QAxObject *range = sheet->querySubObject("Range(const QString&)", cell);
	range->dynamicCall("SetValue(const QString&)", value);
}
...
void QExcel::setCellTextPos(const QString &cell, EnumTextAliPos pos)
{
	QAxObject *range = sheet->querySubObject("Range(const QString&)", cell);
	range->setProperty("HorizontalAlignment", pos);//xlCenter  
}
