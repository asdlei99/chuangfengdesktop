...
void MatterObject::run()
{
	...
	QExcel::copyFileToPath("material.xlsx", m_strTablePath,true);
...
// 	object.setCellString("C3", QString::fromLocal8Bit("领用"));
...
// 	object.setCellString("H3", QString::fromLocal8Bit("本期分摊"));
	...
	object.setCellString(iInsertRaw, 2, QString::number(m_initAmount));
	iInsertRaw++;
	inData += m_initAmount;

	//object.setCellString(iInsertRaw, 1, QString::fromLocal8Bit("本月购入"));
	object.setCellString(iInsertRaw, 2, QString::number(m_currentAmout));
	iInsertRaw++;
	...
	object.setCellString(iInsertRaw, 4, QString::number(inData - outData));
	object.setBorder(m_iDataStartRaw - 1, iInsertRaw, 1, 4);

	for (auto&kvp : m_fixedList)
	{
		object.setCellString(iSubInsertRaw, 6, kvp.first);
		object.setCellString(iSubInsertRaw, 7, QString::number(get<0>(kvp.second)));
		subinData += get<0>(kvp.second);
		object.setCellString(iSubInsertRaw, 8, QString::number(get<1>(kvp.second)));
		subOutData += get<1>(kvp.second);
		iSubInsertRaw++;
	}

	object.setCellString(iSubInsertRaw, 6, QString::fromLocal8Bit("合计："));
	object.setCellString(iSubInsertRaw, 7, QString::number(subinData));
	object.setCellString(iSubInsertRaw, 8, QString::number(subOutData));

	object.setBorder(m_iDataStartRaw - 1, iSubInsertRaw, 6, 9);
	object.save();
}
